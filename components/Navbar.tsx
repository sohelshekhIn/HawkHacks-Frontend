    ...
    >
      <nav className="w-full relative rounded-full border border-white/20 backdrop-filter backdrop-blur-lg shadow-input flex justify-center space-x-5 px-3 py-3 sm:py-4 ">
        <Link href="/" className="flex items-center pl-1 lg:mr-12">
          <span className="text-md font-bold text-white">404</span>
        </Link>
        {navItems.map(
          (
            navItem: {
            ...
            <Link key={`link-${id}`} href={navItem.link} className={cn(
        ...
        <span className="flex items-center lg:ml-12 lg:pl-12">
          <button className="relative inline-flex h-10 overflow-hidden rounded-full p-[1px] px-[0.9px] focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
